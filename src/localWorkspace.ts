...
list_workspaces, open_workspace, refresh_workspace and close_workspace remain for recovering older saved working copies. Do not use them as a prerequisite for direct path access, or discard any unreturned changes. Report actual tool failures honestly.`,
  tools: [
    ...
    { name: 'publish_workspace_file', description: 'Store ONE local text document (HTML, Markdown, JSON, CSV, plain text) on the platform in a single call, and return its `rn` resource name, id and shareable url. USE THIS to show a local HTML file in the live browser — publish it, then call browser_set_content with the returned `rn` as htmlRef; the browser runs in an isolated container and can never open a file:// path, a localhost address, or a server you start. Also use it to give the user a link to a local document. The file is read server-side: do NOT read, chunk, base64 or re-send its contents yourself. Up to 5 MB; binary files (images, PDFs) are not supported yet. Nothing is written to the user\u2019s file.', input_schema: schema({
