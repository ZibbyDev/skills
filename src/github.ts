    ...
    try {
      switch (name) {
        ...
        case 'github_list_pr_comments': {
          ...
          ].sort((a: any, b: any) => (new Date(a.createdAt) as any) - (new Date(b.createdAt) as any));
        ...
        case 'github_create_review': {
          ...
          const inline = Array.isArray(comments)
            ? comments
                ...
                .map((c) => {
                  const out: any = { path: c.path, body: String(c.body) };
                  if (c.line != null) {
          ...
          const deduped = toPost.map(({ _fp, ...c }: any) => c); // keep marker in body, drop the helper field
