...
const CURATED_OXLINT_CONFIG: any = {
...
const CURATED_SEMGREP_RULES: any = {
  rules: [
    {
...
function runScanner(scanner, baseDir, absFiles) {
  ...
  if (res.error) {
    const why = ( res.error as any).code === 'ENOENT' ? `binary not installed (${bin})` : String(res.error.message || res.error);
