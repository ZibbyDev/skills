 ...
 */
export async function planeFetch(path, opts: any = {}) {
...
function matchState(states, target) {
  ...
  for (const [, words] of (Object.entries(aliasGroups) as [string, any][])) {
...
export const planeAdapter: any = {
   ...
   */
  async addComment(key, body, ctx: any = {}) {
    const { workspaceSlug, projectId } = resolveScope(ctx);
   ...
   */
  async transition(key, targetStateName, ctx: any = {}) {
    const { workspaceSlug, projectId } = resolveScope(ctx);
   ...
   */
  async linkPullRequest(key, prUrl, title, ctx: any = {}) {
