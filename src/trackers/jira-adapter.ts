...
const STATUS_CATEGORY_TO_BUCKET: any = {
  new: 'todo',
...
function toNeutral(issue, instanceUrl?: any) {
...
export const jiraAdapter: any = {
  ...
  async listCandidates(opts: any = {}) {
    ...
    const data = await (jiraFetch as any)(`/rest/api/3/search/jql?${qs}`);
  ...
  async getTicket(key) {
    ...
    const issue = await (jiraFetch as any)(`/rest/api/3/issue/${encodeURIComponent(key)}`);
