...
const EXT_TO_KIND = (() => {
  const m: any = {};
  for (const [kind, exts] of (Object.entries(EXT_KIND) as [string, any][])) for (const e of exts) m[e] = kind;
  return m;
})();
 ...
 */
export function commitFacts(files, opts: any = {}) {
  const list = Array.isArray(files) ? files : [];
  const paths = [];
  const extCounts: any = {};
  const kindCounts: any = { code: 0, test: 0, docs: 0, config: 0, style: 0, data: 0, other: 0 };
  let totalAdd = 0;
  let totalDel = 0;
  let largest: any = { path: '', changed: -1 };
  for (const f of list) {
...
export const codeStatsSkill: any = {
  id: 'code-stats',
