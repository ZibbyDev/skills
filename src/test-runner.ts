...
export const testRunnerSkill: any = {
  id: 'runner',
...
function handleRunStatus(args) {
  ...
  if (runId === 'all') {
    const runs = [...activeRuns.entries()].map(([id, r]: any) => {
    ...
    const result: any = { total: runs.length, running, passed, failed, runs };
...
function diagnoseFromSignals({ run, logTail, errorTail }: any) {
  const text = `${logTail || ''}\n${errorTail || ''}`;
